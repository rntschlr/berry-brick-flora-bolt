...
test("never injects the grok.com extensions script (public Magdolna)", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", {
    appName: "Demo",
    projectId: "",
    site: {},
    cwd: mkdtempSync(join(tmpdir(), "grok-og-ext-")),
  });
  assert.doesNotMatch(out, /grok-app-builder\/extensions\.js/);
...
test("skips grok.com extensions even when a project id is provided", () => {
  assert.equal(isPublicStandaloneBuild(), true);
  const out = injectGrokPwaHead("<html><head></head></html>", {
    appName: "Magdolna",
    projectId: "proj-123",
    site: {},
    cwd: mkdtempSync(join(tmpdir(), "grok-og-pub-")),
  });
  assert.doesNotMatch(out, /grok-app-builder\/extensions\.js/);
  assert.doesNotMatch(out, /grok-project-id/);
  assert.doesNotMatch(out, /property="grok:app_id"/);
  assert.match(out, /theme-color" content="#F4EFE4"/);
});

test("does not inject grok project id metas on public Magdolna", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", {
    appName: "Demo",
    projectId: "proj-123",
    site: {},
    cwd: mkdtempSync(join(tmpdir(), "grok-og-pid-")),
  });
  assert.doesNotMatch(out, /grok-project-id/);
  assert.doesNotMatch(out, /data-project-id/);
  assert.doesNotMatch(out, /property="grok:app_id"/);
...
test("does not inject grok:app_id at all", () => {
  const ctx = {
    appName: "Demo",
    projectId: "proj-123",
    site: {},
    cwd: mkdtempSync(join(tmpdir(), "grok-og-dup-")),
  };
  const once = injectGrokPwaHead("<html><head></head></html>", ctx);
  const twice = injectGrokPwaHead(once, ctx);
  assert.equal(once, twice);
  assert.equal(twice.split('property="grok:app_id"').length - 1, 0);
});
...
test("document title entities are not double-escaped on og:title", () => {
  const out = injectGrokPwaHead(
    ...
    { site: {}, cwd: mkdtempSync(join(tmpdir(), "grok-og-entities-")) },
  );
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = createHeadInjector({
    appName: "Wild Race",
    site: {},
    cwd: mkdtempSync(join(tmpdir(), "grok-og-stream-")),
  });
  const chunks = [
...
test("does not inject the extensions script", () => {
  const ctx = {
    ...
    projectId: "proj-123",
    site: {},
    cwd: mkdtempSync(join(tmpdir(), "grok-og-ext2-")),
  };
  const once = injectGrokPwaHead("<html><head></head></html>", ctx);
  const twice = injectGrokPwaHead(once, ctx);
  assert.equal(once, twice);
...
test("names the install page from host slug", () => {
  assert.equal(appNameFromHost("localhost:8080"), "Magdolna");
  assert.equal(appNameFromHost("172.17.154.217:8080"), "Magdolna");
  assert.equal(appNameFromHost("wild-race.grok.me"), "Wild Race");
});

test("rejects hosts that are not plain slugs", () => {
  assert.equal(appNameFromHost("<script>alert(1)</script>"), "Magdolna");
  assert.equal(appNameFromHost('"><img src=x onerror=1>.grok.me'), "Magdolna");
});
